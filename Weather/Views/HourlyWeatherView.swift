...
struct HourlyWeatherView: View {
    let defaultDateStr = "____年____月____日"
    ...
    var body: some View {
        ...
        if let forecastsDay = weatherVM.forecast?.forecastsDay {
            ...
            ForEach(forecastsDay, id: \.self) { forecastDay in  // 3回繰り返す(3日分)
                ...
                let hourlyDisplayForecasts = forecastDay.hour.compactMap {
                    hourlyDisplayForecast in
                    hourlyDisplayForecast.toDisplayFormat(
                        hourlyForecast: hourlyDisplayForecast
                    )
                }
                            ...
                            HStack(spacing: 5) {
                                ForEach(hourlyDisplayForecasts) {
                                    ...
                                    VStack(spacing: 10) {
                                        ...
                                        AsyncImageView(
                                            urlStr:
                                                "https:\(hourlyDisplayForecast.weatherIcon)"
                                        )
